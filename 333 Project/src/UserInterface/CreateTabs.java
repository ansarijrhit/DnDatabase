...
public class CreateTabs extends Tabs {
	private UIMain UI;
	private boolean enablePlayer;
	private boolean enableDM;

	public CreateTabs(UIMain ui, boolean enablePlayer, boolean enableDM) {
		this.UI = ui;
	...
	protected void createTabs() {
		...
		initilizeMajorCharacterViewPlayer(majorCharacter, new GridBagConstraints());
		majorCharacter.setVisible(true);
		this.addTab("Major Character - PC", majorCharacter);

		if (enableDM) {
			JPanel majorCharacterDM = new JPanel(new GridBagLayout());
			initilizeMajorCharacterViewDM(majorCharacterDM, new GridBagConstraints());
			majorCharacterDM.setVisible(true);
			this.addTab("Major Character - NPC", majorCharacterDM);
	...
	private void initilizeMajorCharacterViewPlayer(JPanel panel, GridBagConstraints c) {
		panel.removeAll();
		panel.repaint();
		JComboBox<Object> campaignIds = new JComboBox<Object>(UI.getAllCampaignIds());
		JTextField name = new JTextField(30);
		JComboBox<Object> races = new JComboBox<Object>(UI.getPossibleRaces());
		...
		submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cCampaignID.isBlank() || cName.isBlank() || cRace.isEmpty() || cClass.isEmpty() || cLevel.isBlank()
					...
					if (result == JOptionPane.OK_OPTION) {
						boolean success = UI.getBackEnd().getCreateFunctions().createMajorCharacter(cClass, cLevel,
								cHitpoints, cAlignment, cBackground, UI.getPlayerUsername(), cName, cRace, "",
								cCampaignID);
						if (success) {
				...
				initilizeMajorCharacterViewPlayer(panel, new GridBagConstraints());
			}
		});
		panel.add(submit, c);
	}

	private void initilizeMajorCharacterViewDM(JPanel panel, GridBagConstraints c) {
		panel.removeAll();
		panel.repaint();
		...
		submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cCampaignID.isBlank() || cName.isBlank() || cRace.isEmpty() || cClass.isEmpty() || cLevel.isBlank()
					...
					if (result == JOptionPane.OK_OPTION) {
						boolean success = UI.getBackEnd().getCreateFunctions().createMajorCharacter(cClass, cLevel,
								cHitpoints, cAlignment, cBackground, "", cName, cRace, UI.getPlayerUsername(),
								cCampaignID);
						if (success) {
							JOptionPane.showMessageDialog(panel, "Major Character was added to the database.");
				...
				initilizeMajorCharacterViewDM(panel, new GridBagConstraints());
			}
		});
		panel.add(submit, c);
	}
		...
		submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cName.isBlank()) {
					...
					int result = JOptionPane.showConfirmDialog(panel, confirmString, "Confirm Creation", optionType);
					if (result == JOptionPane.OK_OPTION) {
						boolean success = UI.getBackEnd().getCreateFunctions().createCampaign(UI.getPlayerUsername(),
								cName);
						if (success) {
							JOptionPane.showMessageDialog(panel, "Campaign was added to the database.");

						} else {
							JOptionPane.showMessageDialog(panel, "Error: Unable to add Campaign to the database.");
						}
					} else {
						return;
					}
		...
		submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cName.isBlank() || cDescription.isBlank() || cCampaignId.isEmpty()) {
					...
					int result = JOptionPane.showConfirmDialog(panel, confirmString, "Confirm Creation", optionType);
					if (result == JOptionPane.OK_OPTION) {
						boolean success = UI.getBackEnd().getCreateFunctions().createNotes(UI.getPlayerUsername(),
								cCampaignId, cName, cDescription);
						if (success) {
							JOptionPane.showMessageDialog(panel, "Note was added to the database.");

						} else {
							JOptionPane.showMessageDialog(panel, "Error: Unable to add Note to the database.");
						}
					} else {
						return;
					}
		...
		submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cName.isBlank() || cDescription.isBlank() || cCampaignId.isEmpty()) {
					...
					int result = JOptionPane.showConfirmDialog(panel, confirmString, "Confirm Creation", optionType);
					if (result == JOptionPane.OK_OPTION) {
						boolean success = UI.getBackEnd().getCreateFunctions().createLocation(UI.getPlayerUsername(),
								cName, cDescription, cCampaignId);
						if (success) {
							JOptionPane.showMessageDialog(panel, "Location was added to the database.");

						} else {
							JOptionPane.showMessageDialog(panel, "Error: Unable to add Location to the database.");
						}
					} else {
						return;
					}
